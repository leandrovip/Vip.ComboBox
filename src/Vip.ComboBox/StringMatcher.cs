    ...
    public class StringMatcher
    {
        ...
        private StringMatch buildNoWildcard(string source)
        {
            ...
            for (j = 0; j < NP; j++)
            {
                ...
                if (i >= NS) break; // j loop
            }

            if (!isMatch) return null;
            if (i < NS) segments.Add(source.Substring(i, NS - i));
        ...
        private StringMatch buildWithWildcards(string source)
        {
            ...
            if (iS != 0) addChunkToStringMatch(res, source.Substring(0, iS), true);
                    ...
                    if (pattern[iP] == '*')
                    {
                        ...
                        if (icP != iP)
                        {
                            ...
                            if (icP >= 0)
                            {
                                ...
                                if (ioS1 != ioS2) addChunkToStringMatch(res, source.Substring(ioS1, ioS2 - ioS1), false);
                                if (ioS2 != iS) addChunkToStringMatch(res, source.Substring(ioS2, iS - ioS2), true);
                            }
            ...
            if (iP == NP)
            {
                ...
                if (icP != iP)
                    if (ioS1 != -1)
                    {
                        ...
                        {
                            if (ioS2 != ioS1) addChunkToStringMatch(res, source.Substring(ioS1, ioS2 - ioS1), false);
                            if (ioS2 != iS) addChunkToStringMatch(res, source.Substring(ioS2, iS - ioS2), true);
                        }
                    }

                if (iS != NS) addChunkToStringMatch(res, source.Substring(iS, NS - iS), false);
                return res;
            }
        ...
        private StringMatch buildRegex(string source)
        {
            ...
            if (m.Success)
            {
                ...
                if (m.Index == idx)
                {
                    if (m.Index == 0) startsOnMatch = true;
            ...
            if (idx < source.Length) segments.Add(source.Substring(idx, source.Length - idx));
